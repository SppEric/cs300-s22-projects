...
bool synchronized_queue<T>::pop(T* elt) {
  // Lock down the critical code for this function
  std::unique_lock<std::mutex> lock(this->mtx);

  // Wait until there is an element in the queue
  while (!this->is_stopped.load() && !this->q.size()) {
    this->cv.wait(lock);
  }

  //  Check if the queue is out of scope
  ...
  *elt = this->q.front();
  ...
  // Return that the queue has not been stopped
  return false;
}
...
void synchronized_queue<T>::push(T elt) {
  ...
  bool is_empty = (this->q.size() == 0);

  // Push an element into the back of the queue
  this->q.push(elt);

  // If the queue was empty, then notify a thread
  if (is_empty) {this->cv.notify_one();}
}
...
std::vector<T> synchronized_queue<T>::flush() {
  ...
  // For each element in the queue, pop and then add it to the elts vector
  int queue_size = this->q.size();
