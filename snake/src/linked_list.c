...
void insert_first(node_t** head_list, void* to_add, size_t size) { 
  ...
  if (!(*head_list)) { // Given list is empty!
...
void* get(node_t* head_list, int index) { 
  if (!head_list) { // If list is empty
    return NULL;
  }

  if (length_list(head_list) <= index || index < 0) {
    return NULL;
  }
  ...
  while (index > 0) { // Iterate to the desired location
    curr = curr->next;
    index--;
  }
...
void* remove_first(node_t** head_list) {
  ...
  *head_list = (*head_list)->next;
  ...
  if (*head_list) { // If the next element exists
    (*head_list)->prev = NULL;
  }

  free(curr); // Free the unused node
...
void* remove_last(node_t** head_list) { 
  ...
  node_t* curr = *head_list;
  while (curr->next) { // Iterate to last element in the list
    curr = curr->next;
  }
  
  if (curr->prev) { // If the previous element exists
    curr->prev->next = NULL;
  }
  else {
    *head_list = NULL;
  ...
  // Save pointer to data before deleting
  void* last_elem_pointer = curr->data;  

  // Free unused node
  free(curr);

  return last_elem_pointer; 
}
